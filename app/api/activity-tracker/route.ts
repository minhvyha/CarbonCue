...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ error: "Missing activity type or input" }, { status: 401 });
        }

        // 3. Fetch prediction from AI model
        const endpoint = Activity_MAP[activityType as keyof typeof Activity_MAP];
        if (!endpoint) {
            return NextResponse.json({ error: "Invalid activity type" }, { status: 402 });
        }
        ...
        const response = await fetch(`https://carboncueapi-production-1664.up.railway.app/${endpoint}`, {
            method: 'POST',
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({ error: "Prediction API failed", details: errData }, { status: 500 });
        }
        ...
        return NextResponse.json({ message: "Activity logged successfully", saved }, { status: 201 });
    } catch (error) {
...
export async function GET(request: NextRequest) {
    try {
        ...
        const activityLogs = await ActivityLog.find({ userId: user._id }).sort({ timestamp: -1 });
